...
// Tool: getLowStockReport - Productos por debajo de threshold o con riesgo de quiebre según proyección simple
export async function getLowStockReport() {
  ...
  const lowStockProducts = products.filter((p) => {
    ...
    return p.stock < p.threshold || p.stock < proyeccion7d;
  });
...
export async function getOverstockReport() {
  ...
  const overstockProducts = products.filter((p) => {
    ...
    const horizonte45d = promedioDiario30d * 45; // Horizonte de 45 días
    const ventasBajas = p.sales_last_30 < 5; // Definir "bajas" como < 5 en 30 días
    return p.stock > horizonte45d && ventasBajas;
  });
